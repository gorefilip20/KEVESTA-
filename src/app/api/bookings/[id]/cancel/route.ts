...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (booking.status !== "paid") {
      ...
      await notifyBookingEvent({ userId: booking.user_id, bookingId: booking.id, eventType: "booking_cancelled", title: "Booking cancelled", body: `${booking.item_title} has been cancelled.` }).catch((error) => console.error("Booking cancellation notification failed", error));
    ...
    if (payment.provider === "column") {
      ...
      await notifyBookingEvent({ userId: booking.user_id, bookingId: booking.id, eventType: "refund_requested", title: "Refund request received", body: `${booking.item_title} · ${money(booking.amount_cents, booking.currency)} is now refund-pending.` }).catch((error) => console.error("Refund request notification failed", error));
