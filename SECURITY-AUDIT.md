...
| BOOK-02 | High | Booking confirmation is not persisted as a durable state machine. | Payment and inventory can diverge after retries, restarts, or webhook reordering. | Persist booking and payment records with unique provider IDs, explicit transitions, and transactional inventory reservation/release. |
...
| PAY-02 | Medium | Webhook duplicate protection is process-local. | Duplicates can reappear across restarts or multiple instances. | Persist `event.id` under a database unique constraint before mutating booking state. |
| PAY-03 | Medium | Webhook events can arrive out of order. | A stale event could regress a booking if transitions are not monotonic. | Store provider timestamps/status ranks and reject state regressions. |
...
The Column handler uses the official `Column-Signature` header and raw-body HMAC verification. It requires the stable event ID supplied by Column and acknowledges duplicate deliveries without processing them twice within a warm process. Durable event persistence remains required before production scale-out.
