...
const httpServer = http.createServer((request, response) => {
  ...
  if (request.method === "POST" && (request.url?.startsWith("/v1/ach/transfers") || request.url?.startsWith("/v1/ach/refunds"))) { let body = ""; request.on("data", (chunk) => { body += chunk; }); request.on("end", () => { response.writeHead(200, { "Content-Type": "application/json" }); response.end(JSON.stringify({ id: `${request.url?.includes("refund") ? "refund" : "col"}_local_${Date.now()}`, status: "processing", received: JSON.parse(body || "{}") })); }); return; }
...
async function main() {
  ...
  const child = spawn("npm", ["run", "dev", "--", "--hostname", "127.0.0.1", "--port", "3001"], { stdio: "inherit", env: { ...process.env, DATABASE_URL: "pg-mem://local-integration", COLUMN_API_KEY: "local-column-key", COLUMN_API_BASE: "http://127.0.0.1:4010", COLUMN_ACH_TRANSFER_PATH: "/v1/ach/transfers", COLUMN_REFUND_PATH: "/v1/ach/refunds", COLUMN_RECEIVING_ACCOUNT_ID: "local-receiving-account", COLUMN_WEBHOOK_SECRET: "local-column-secret", SMTP_HOST: "127.0.0.1", SMTP_PORT: String(smtpAddress.port), SMTP_USER: "local", SMTP_PASSWORD: "local", EMAIL_FROM: "KEVESTA <local@kevesta.test>", NEXT_PUBLIC_APP_URL: "http://127.0.0.1:3001", NODE_ENV: "test", LOCAL_INTEGRATION: "1" } });
