...
test("local mock stack runs the authenticated ACH lifecycle end to end", async ({ request }) => {
  ...
  const bookings = await request.get("/api/bookings", { headers: { Cookie: cookie } });
  const booking = (await bookings.json()).bookings[0];
  const refundRequest = await request.post(`/api/bookings/${booking.id}/cancel`, { headers: { Cookie: cookie }, data: { reason: "local integration test" } });
  expect(refundRequest.status()).toBe(202);
  const refundBody = await refundRequest.json();
  expect(refundBody.status).toBe("refund_pending");
  expect(refundBody.receiptNumber).toMatch(/^RF-/);
  const refundEvent = { id: `evt_refund_local_${Date.now()}`, type: "ach.refund.completed", data: { transfer_id: paymentBody.payment.paymentId, refund_id: "refund_local_confirmed", status: "completed" } };
  const refundRaw = JSON.stringify(refundEvent);
  const refundSignature = crypto.createHmac("sha256", "local-column-secret").update(refundRaw).digest("hex");
  const completed = await request.post("/api/payments/webhook", { headers: { "Column-Signature": refundSignature }, data: refundEvent });
  expect((await completed.json()).status).toBe("refunded");
  const emails = await (await request.get("http://127.0.0.1:4011/__test/emails")).json();
  const parsedCompleted = await simpleParser(emails.messages.at(-1) || "");
  expect(parsedCompleted.subject).toBe("Your KEVESTA refund is complete");
  expect(parsedCompleted.html).toContain("refund_local_confirmed");
});
